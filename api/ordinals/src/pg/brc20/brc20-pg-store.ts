...
export class Brc20PgStore extends BasePgStore {
  ...
  private trackQuery<T>(operationType: string, fn: () => Promise<T>): Promise<T> {
    ...
    return fn().finally(() => {
      ...
      if (typeof tracker.trackDbQuery === 'function') {
        tracker.trackDbQuery(`brc20_${operationType}`, startTime);
      }
    });
  }
  ...
  async getTokens(
    ...
    return this.trackQuery('get_tokens', async () => {
  ...
  async getBalances(
    ...
    return this.trackQuery('get_balances', async () => {
      const ticker = sqlOr(
        this.sql,
  ...
  async getTransferableInscriptions(
    ...
    return this.trackQuery('get_transferable_inscriptions', async () => {
  ...
  async getToken(args: { ticker: string }): Promise<DbBrc20TokenWithSupply | undefined> {
    return this.trackQuery('get_token', async () => {
  ...
  async getTokenHolders(
    ...
    return this.trackQuery('get_token_holders', async () => {
  ...
  async getActivity(
    ...
    return this.trackQuery('get_activity', async () => {
